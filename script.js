...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function generateCalendar(year, month) {
        ...
        <div class="calendar-month"><b>${monthNames[month]}</b></div>
        <div class="calendar-year"><b>${year}</b></div>
        </div>`;
        let body = `<div class="calendar-days">`;

        for (let i = 0; i < firstDayIndex; i++) {
            body += `<div class="calendar-day empty"></div>`;
        }

        for (let day = 1; day <= daysInMonth; day++) {
            const today = new Date();
            const isToday = day === today.getDate() && month === today.getMonth() && year === today.getFullYear();
            body += `<div class="calendar-day ${isToday ? 'today' : ''}">${day}</div>`;
        }

        body += `</div>`;

        calendarContainer.innerHTML = header + body;
    }
